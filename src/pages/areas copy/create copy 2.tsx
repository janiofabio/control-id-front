...
export const AreaCreate: React.FC = () => {
  ...
  const watchControlarCapacidade = watch("controlarCapacidade");
  ...
  const onSubmit = async (dados: any) => {
    try {
      ...
      const usuarioAtual = "Usuário Atual"; // Substituir pela informação real do usuário
    ...
    <Create saveButtonProps={{ onClick: handleSubmit(onSubmit) }}>
      <Box sx={{ padding: 2 }}>
        ...
        <Box sx={{ mt: 2 }}>
          ...
          <Typography variant="body2">
            Última atualização por: {"{atualizadoPor}"} em {"{atualizadoEm}"}
          </Typography>
        </Box>
      </Box>
    </Create>
