...
export const GroupEdit = () => {
  ...
  const { data: groupData, isLoading } = useOne({
    resource: "groups",
    id: id as string,
  });
  ...
  useEffect(() => {
    if (groupData?.data) {
      ...
      reset({
        ...group,
        people: group.people?.map((person: any) => person.id) || [],
      });
      setSelectedPeople(group.people?.map((person: any) => person.id) || []);
    }
  }, [groupData, reset]);
  ...
  const handleClear = () => {
    reset({ description: groupData?.data?.description || "", ignoreAntiDoubleEntry: groupData?.data?.ignoreAntiDoubleEntry || false });
  };
