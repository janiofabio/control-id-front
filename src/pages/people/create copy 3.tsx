...
export const PersonCreate: React.FC = () => {
    ...
    const { saveButtonProps, refineCore: { onFinish, formLoading }, control, register, handleSubmit, formState: { errors }, setValue, watch, trigger } = useForm();
    ...
    // Função modificada para salvar com logs no console
    const handleSave = (data: any) => {
        console.log("Form data being saved:", data); // Exibe os dados no console
        onFinish(data); // Chama a função original de finalização
    };

    return (
        <Create isLoading={formLoading} saveButtonProps={{ onClick: handleSubmit(handleSave) }}>
            <Box sx={{ padding: 2 }}>
