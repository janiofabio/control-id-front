...
export const PersonEdit = () => {
  ...
  const [preview, setPreview] = useState<string>(peopleData?.photouser || "");

  useEffect(() => {
    if (peopleData?.photouser) {
      setPreview(peopleData.photouser); // Exibe a foto existente ao editar
    }
  }, [peopleData]);
    ...
    if (file) {
      ...
      try {
        ...
        if (response.ok && data[0]) {
          ...
          setValue("photouser", uploadedFileUrl); // Salva a URL da foto
        } else {
        ...
        <Box sx={{ padding: 2 }}>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={9}>
              <PersonFormFields register={register} errors={errors} peopleData={peopleData} getValues={getValues} setValue={setValue} />
            </Grid>
          </Grid>
